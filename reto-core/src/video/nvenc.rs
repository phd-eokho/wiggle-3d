...
struct CudaContextDriver {
    lib: *mut c_void,
    ctx: *mut c_void,
    ctx_destroy: Option<CuCtxDestroyV2Fn>,
}

unsafe fn load_cuda_context() -> Result<CudaContextDriver, VideoError> {
    unsafe {
        let cuda_names = [
            #[cfg(target_os = "windows")]
            "nvcuda.dll",
        ...
        Ok(CudaContextDriver {
            lib: cuda_lib,
            ctx: cu_ctx,
            ctx_destroy: cu_ctx_destroy,
        })
    }
}

struct NvencFunctionTable {
    lib: *mut c_void,
    funcs: NV_ENCODE_API_FUNCTION_LIST,
    api_version: u32,
}
...
unsafe fn load_nvenc_function_table() -> Result<NvencFunctionTable, VideoError> {
    unsafe {
        let nvenc_names = [
            #[cfg(target_os = "windows")]
            "nvEncodeAPI64.dll",
...
impl DynamicNvencDriver {
    fn load() -> Result<Self, VideoError> {
        unsafe {
            let cuda = load_cuda_context()?;
            let nvenc = match load_nvenc_function_table() {
                Ok(tbl) => tbl,
                Err(e) => {
                    if let Some(destroy) = cuda.ctx_destroy {
                        destroy(cuda.ctx);
                    }
                    dyn_close_lib(cuda.lib);
                    return Err(e);
                }
            };
            ...
            Ok(Self {
                _cuda_lib: cuda.lib,
                _nvenc_lib: nvenc.lib,
                cu_ctx: cuda.ctx,
                cu_ctx_destroy: cuda.ctx_destroy,
                funcs: nvenc.funcs,
                api_version: nvenc.api_version,
            })
        }
    }
...
impl HevcFrameEncoder for NvencHevcEncoder {
    fn initialize(&mut self, config: &HevcEncoderConfig) -> Result<(), VideoError> {
        ...
        unsafe {
            let encoder_ptr = open_nvenc_session(&driver)?;
            ...
            self.bitstream_buffer = bitstream_buf;
            self.frame_index = 0;
    ...
    fn encode_frame(
        ...
        unsafe {
            let pitch = upload_yuv_planar_frame(driver, self.encoder, self.input_buffer, frame)?;
            let params = PictureEncodeParams {
                frame,
                pitch,
                is_keyframe,
                frame_index: self.frame_index,
            };
            let ready = execute_encode_picture(
                driver,
                self.encoder,
                self.input_buffer,
            ...
            extract_bitstream_nalus(driver, self.encoder, self.bitstream_buffer)
        }
    }
...
struct PictureEncodeParams<'a> {
    frame: &'a Yuv420PlanarFrame,
    pitch: u32,
    is_keyframe: bool,
    frame_index: u32,
}

unsafe fn open_nvenc_session(driver: &DynamicNvencDriver) -> Result<*mut c_void, VideoError> {
    unsafe {
...
unsafe fn query_preset_and_config(
    driver: &DynamicNvencDriver,
    encoder_ptr: *mut c_void,
    config: &HevcEncoderConfig,
) -> (GUID, Option<NV_ENC_PRESET_CONFIG>) {
    unsafe {
        let struct_ver = |ver: u32| -> u32 { driver.api_version | (ver << 16) | (0x7 << 28) };
        ...
        if has_preset_cfg {
            ...
            (selected_preset, Some(preset_cfg))
        } else {
...
unsafe fn init_nvenc_encoder(
    driver: &DynamicNvencDriver,
    encoder_ptr: *mut c_void,
    config: &HevcEncoderConfig,
    selected_preset: GUID,
    mut preset_cfg: Option<NV_ENC_PRESET_CONFIG>,
) -> Result<(), VideoError> {
    unsafe {
        let struct_ver = |ver: u32| -> u32 { driver.api_version | (ver << 16) | (0x7 << 28) };
        ...
        if let Some(cfg) = preset_cfg.as_mut() {
...
unsafe fn allocate_nvenc_buffers(
    driver: &DynamicNvencDriver,
    encoder_ptr: *mut c_void,
    config: &HevcEncoderConfig,
) -> Result<(*mut c_void, *mut c_void), VideoError> {
    unsafe {
        let struct_ver = |ver: u32| -> u32 { driver.api_version | (ver << 16) | (0x7 << 28) };
        ...
        Ok((create_in.inputBuffer, create_bs.bitstreamBuffer))
    }
}

unsafe fn upload_yuv_planar_frame(
    driver: &DynamicNvencDriver,
    encoder: *mut c_void,
    input_buffer: *mut c_void,
    frame: &Yuv420PlanarFrame,
    ...
    unsafe {
        ...
        lock_in.inputBuffer = input_buffer;
        ...
        let status = lock_input(encoder, &mut lock_in);
        if status != NV_ENC_SUCCESS {
            let err_str = driver.get_last_error(encoder);
            return Err(VideoError::NvencError(format!(
        ...
        unlock_input(encoder, input_buffer);

        Ok(lock_in.pitch)
    }
}

unsafe fn execute_encode_picture(
    driver: &DynamicNvencDriver,
    encoder: *mut c_void,
    input_buffer: *mut c_void,
    bitstream_buffer: *mut c_void,
    params: &PictureEncodeParams<'_>,
    ...
    unsafe {
        ...
        let mut pic_params: NV_ENC_PIC_PARAMS = std::mem::zeroed();
        pic_params.version = struct_ver(7) | (1 << 31);
        pic_params.inputWidth = params.frame.width;
        pic_params.inputHeight = params.frame.height;
        pic_params.inputPitch = params.pitch;
        ...
        pic_params.frameIdx = params.frame_index;
        if params.is_keyframe || params.frame_index == 0 {
        ...
        let mut status = encode_pic(encoder, &mut pic_params);
        if status == 17 {
            return Ok(false);
        }
        if status != NV_ENC_SUCCESS {
            pic_params.version = struct_ver(4) | (1 << 31);
            status = encode_pic(encoder, &mut pic_params);
            if status == 17 {
                return Ok(false);
            }
        }
...
unsafe fn extract_bitstream_nalus(
    driver: &DynamicNvencDriver,
    encoder: *mut c_void,
    bitstream_buffer: *mut c_void,
) -> Result<Vec<HevcNalUnit>, VideoError> {
    unsafe {
        ...
        lock_bs.outputBitstream = bitstream_buffer;
        ...
        let mut status = lock_bitstream(encoder, &mut lock_bs);
        if status != NV_ENC_SUCCESS {
            lock_bs.version = struct_ver(1);
            status = lock_bitstream(encoder, &mut lock_bs);
        }
        if status != NV_ENC_SUCCESS {
            let err_str = driver.get_last_error(encoder);
            return Err(VideoError::NvencError(format!(
        ...
        unlock_bitstream(encoder, bitstream_buffer);
