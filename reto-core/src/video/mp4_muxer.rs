...
impl Mp4Muxer {
    ...
    pub fn mux_hevc<W: Write>(
        ...
        let ftyp_box = write_box(b"ftyp", &ftyp);

        // 2. Build [mdat] payload and compute sample sizes & sync sample indices
        ...
        for (idx, sample) in samples.iter().enumerate() {
            ...
            for nalu in &sample.nalus {
                ...
                mdat_payload.extend_from_slice(&nalu_len.to_be_bytes()); // 4-byte length prefix (AVCC)
        ...
        // 3. Construct [moov] box
        // Total duration is in timescale units (timescale = 1000)
        ...
        let mvhd_box = write_box(b"mvhd", &mvhd);
        ...
        let tkhd_box = write_box(b"tkhd", &tkhd);
        ...
        let mdhd_box = write_box(b"mdhd", &mdhd);
        ...
        let hdlr_box = write_box(b"hdlr", &hdlr);
        ...
        let vmhd_box = write_box(b"vmhd", &vmhd);
        ...
        let dinf_box = write_box(b"dinf", &dref_box);
        ...
        hvcc.extend_from_slice(&0xF000u16.to_be_bytes()); // min_spatial_segmentation_idc (reserved 4 bits + 0)
        hvcc.push(0xFC); // parallelismType (reserved 6 bits + 0)
        ...
        // Array 1: VPS (NAL 32)
        hvcc.push(0x80 | NAL_VPS); // completeness=1, nalu_type=32
        hvcc.extend_from_slice(&1u16.to_be_bytes()); // count = 1
        hvcc.extend_from_slice(&(vps.len() as u16).to_be_bytes());
        hvcc.extend_from_slice(vps);

        // Array 2: SPS (NAL 33)
        hvcc.push(0x80 | NAL_SPS); // completeness=1, nalu_type=33
        hvcc.extend_from_slice(&1u16.to_be_bytes()); // count = 1
        hvcc.extend_from_slice(&(sps.len() as u16).to_be_bytes());
        hvcc.extend_from_slice(sps);

        // Array 3: PPS (NAL 34)
        hvcc.push(0x80 | NAL_PPS); // completeness=1, nalu_type=34
        hvcc.extend_from_slice(&1u16.to_be_bytes()); // count = 1
        ...
        let stsd_box = write_box(b"stsd", &stsd);

        // --- [stts] (Time-to-Sample) ---
        // Run-length compress identical consecutive durations
        ...
        let stts_box = write_box(b"stts", &stts);
        ...
        let stss_box = write_box(b"stss", &stss);
        ...
        stsc.extend_from_slice(&(samples.len() as u32).to_be_bytes()); // samples_per_chunk = total
        stsc.extend_from_slice(&1u32.to_be_bytes()); // sample_description_index = 1
        let stsc_box = write_box(b"stsc", &stsc);
        ...
        let stsz_box = write_box(b"stsz", &stsz);

        // Compute offsets: [ftyp] -> [moov] -> [mdat]
        // Pre-build stbl with placeholder stco to know exact moov length
        let assemble_moov = |chunk_offset: u32| -> Vec<u8> {
        ...
        // Two-pass chunk offset resolution (stco size is fixed at 16 bytes for 1 chunk)
        let ftyp_len = ftyp_box.len() as u32;
        let dummy_moov = assemble_moov(0);
        let moov_len = dummy_moov.len() as u32;
        // In ISO BMFF, mdat payload starts 8 bytes after mdat header
        let mdat_payload_offset = ftyp_len + moov_len + 8;

        let final_moov = assemble_moov(mdat_payload_offset);

        // 4. Write final MP4 byte stream
